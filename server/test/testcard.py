...
def upgrade_check(client,device_id,imei,imsi):
    try:
        ...
        req.body.version = 1
        req.body.channel = '0'
        req.body.device_id = 'dcb9e1b13bea56c5'
        req.body.imei = 'imei'
...
def texas_sit(client,mobile = '13412341235',password= '123456',token='d_9444'):
    try:
        ...
        # req2 = create_client_message(TexasBetActionReq)
        # req2.header.user = result.header.user
        # req2.body.table_id = tb_id
        ...
        #
        # time.sleep(4)
        # print 'user=%d now public_3 bet pass go public_5' % req.header.user
        # req3 = create_client_message(TexasBetActionReq)
        # req3.header.user = result.header.user
        # req3.body.table_id = tb_id
        # req3.body.bet_type = ADD_BEI
        # req3.body.texas_status = TEXAS_PUBLIC_3
        # req3.body.action_gold = 1000
        # # req3.body.bet_reward_gold = 1000
        # client.socket.send(req3.encode())

        # time.sleep(3)
        # print 'user=%d now public_5 bet gold go over' % req.header.user
        # req3 = create_client_message(TexasBetActionReq)
        # req3.header.user = result.header.user
        # req3.body.table_id = tb_id
        # req3.body.bet_type = PLUS_GOLD
        # req3.body.texas_status = TEXAS_PUBLIC_5
        # req3.body.action_gold = 1000
        # req3.body.bet_reward_gold = 1000
        # client.socket.send(req3.encode())
        ...
        # req3.body.bet_type = PLUS_GOLD
        # req3.body.texas_status = resp.body.table.texas_status
        # req3.body.action_gold = 1000
        # req3.body.bet_reward_gold = 1000
        # client.socket.send(req3.encode())
...
def texas_sit_not_tables(client,mobile = '13412341235',password= '123456',token='d_9444'):
    try:
        MessageMapping.init()
        resp = client.normal_test_enter_server(mobile,password,token)
        ...
        tb_id = resp.body.table.id
        print 'user=%d sit table=%d' % (resp.header.user, tb_id)
        print '5555555555555555555555555555555555'

        req = create_client_message(TexasLeaveTableReq)
        req.header.user = result.header.user
        req.body.table_id = tb_id
        client.socket.send(req.encode())
        resp = client.get_message(client.socket)
        ...
        req.header.user = result.header.user
        req.body.table_id = -1
        req.body.not_tables.append(tb_id)
        # req.body.not_tables = tb_id
        client.socket.send(req.encode())
        resp = client.get_message(client.socket)
    except:
        traceback.print_exc()
...
def test_card(imei,imsi,token,need_idle,*args):
    ...
    try:
        ...
        upgrade_check(client, '17727853917','111111', 'd971ed2bfbb1bd91')
        # texas_sit(client, '18688775086', '123456', 'b0f0dec5c9dc6222')
        #texas_sit_not_tables(client, '18688775086', '123456', 'b0f0dec5c9dc6222')
