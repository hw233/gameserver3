...
class AccessClientConnection:
    _ID_BEGIN = 0x40000000
    _ID_END = 0x90000000

    def __init__(self, access_server, environ):
        self.access_server = access_server
        self.user = -1
        self.environ = environ
        self.ws_sock = environ.get("wsgi.websocket")
        self.is_safe = False
        self.transactions = {}
    ...
    def _run_send(self):
        ...
        while True:
            ...
            if var.DEBUG:
                ...
                logging.info("<== send message back: cmd=%d | user=%d | result=%d " % (
                request.header.command, self.user, request.header.result))
    ...
    def connect_server(self, client_message):
        ...
        if session == req.body.session:
            ...
            if old_server_id != None and int(old_server_id) != self.access_server.server_id:
                ...
                self.access_server.access_service.forward_message_directly(old_server_id, KICKOFF_USER_CONNECTION_ID,
                                                                   req.header.user, -1)
            ...
            self.access_server.redis.hset("server" + str(self.access_server.server_id), req.header.user,
                                          self.connection_id)
    ...
    def on_message(self, client_message):
        ...
        if var.DEBUG:
            ...
            logging.info("==> receive message : cmd=%d | user=%d | route=%d " % (client_message.header.command,
                                                                                 client_message.header.user,
                                                                                 client_message.header.route))
        ...
        if not self.is_safe:
            if client_message.header.command != CONNECT_GAME_SERVER_REQ_ID:
                logging.info(
                    "the first message must be ConnectGamerServer , user = %d" + str(client_message.header.user))
                self.close()
            else:
        ...
        if result < 0:
            logging.info("result(%d) is failure and user = %d, it should not happen", result,
                         client_message.header.user)
            self.close()
    ...
    def on_safe_message(self, client_message):
        ...
        if client_message.header.user in access_server.blacklist_users:
            logging.info(
                "user(%d) in access_server.blacklist, so delete user in access_server.users" % client_message.header.user)
...
class AccessWSServer:
    def __init__(self,server_id,conf,ip,port):
        ...
        # gevent.spawn(WSServer(("0.0.0.0", self.port), self.handle_access_socket, spawn=pool).serve_forever)
        gevent.spawn(WebSocketServer(("0.0.0.0", self.port), self.handle_access_socket, spawn=pool).serve_forever)
        gevent.spawn(self.handle_queue)
        gevent.spawn_later(3, self.kick_user, self.redis)


    def handle_access_socket(self, environ, start_response):
        logging.info("a client connecting now .....")
        buffer = bytearray()

        conn = AccessClientConnection(self, environ)
                ...
                for transaction, client_message in connection.transactions.items():
                    if now - client_message.receive_time > LOOP_PERIOD:
                        ...
                        logging.error(" user %d,message %d ,transaction %d is timeout",
                                      client_message.header.user, client_message.header.command,
                                      client_message.header.transaction)
